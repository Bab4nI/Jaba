...
class UserProgressViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            progress = UserProgress.objects.filter(
                user__in=users, lesson__module__course=course
                    ...
                    response_data["lessons"].append(
                        {
                            "id": lesson.id,
                            "title": lesson.title,
                            "type": lesson.content_type.model
            ...
            for user in users:
                ...
                for lesson in response_data["lessons"]:
                    lesson_progress = progress.filter(
        ...
        return Response(
            {
                ...
                "progress_percentage": (total_current_score / total_max_score * 100)
    ...
    def reset_progress(self, request, lesson_id=None):
        ...
        UserProgress.objects.filter(user=request.user, lesson=lesson).delete()

        return Response(
            {"lesson_id": lesson.id, "message": "Progress reset successfully"}
        )
    ...
    def groups(self, request):
        ...
        groups = (
            User.objects.exclude(groups__isnull=True)
        ...
        try:
            ...
            for user in users:
                try:
                    ...
                    for lesson in lessons:
                        ...
                        for progress in progress_records:
                            if progress.content:  # Content-level progress
                                ...
                                total_max_score += (
                                    progress.max_score or DEFAULT_MAX_SCORE
                                )
                    ...
                    logger.error(
                        f"Error processing user {user.id}: {str(e)}", exc_info=True
                    )
