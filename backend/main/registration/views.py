    ...
    def post(self, request):
        try:
            ...
            html_message = render_to_string(
                "registration/registration_email.html", context
            )
...
class PasswordResetRequestView(APIView):
    def post(self, request):
        ...
        reset_url = (
            f"{settings.FRONTEND_URL}/New_password?token={str(token.access_token)}"
        )

        # Отправка email
        context = {"user": user, "reset_url": reset_url, "site_name": "NetLab AI"}

        try:
            html_message = render_to_string(
                "registration/password_reset_e.html", context
            )
            plain_message = strip_tags(html_message)
...
class PasswordResetConfirmView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"status": "Пароль успешно изменен"}, status=status.HTTP_200_OK
            )
...
class ChangeEmailRequestView(APIView):
    def post(self, request):
        ...
        context = {"code": verification_code, "site_name": "NetLab AI"}

        try:
            subject = "Подтверждение смены email"
            html_message = render_to_string(
                "registration/email_verification.html", context
            )
            plain_message = strip_tags(html_message)
...
class VerifyEmailCodeView(APIView):
    def post(self, request):
        ...
        if (
            ...
            or user.email_verification_code_expires < timezone.now()
        ):
            return Response(
        ...
        return Response({"status": "Email успешно изменен"}, status=status.HTTP_200_OK)
